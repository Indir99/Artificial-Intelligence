...
class Maze:
    ...
    actions = [Action.MOVE_LEFT, Action.MOVE_RIGHT,
               Action.MOVE_UP, Action.MOVE_DOWN]  # all possible actions

    reward_exit = 20.0  # reward for reaching the exit cell
    penalty_move = -0.2  # penalty for a move which did not result in finding the exit cell
    # mijenjali smo ovo
    penalty_visited = -0.4  # penalty for returning to a cell which was visited earlier
    # penalty for trying to enter an occupied cell or moving out of the maze
    penalty_impossible_move = -1.6
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None):
        ...
        self.__exit_cell = (ncols - 1, nrows -
    ...
    def __draw(self):
        ...
        self.__ax1.plot(*zip(*[self.__previous_cell, self.__current_cell]),
                        "bo-")  # previous cells are blue dots
        # current cell is a red dot
    ...
    def step(self, action):
        ...
        logging.debug("action: {:10s} | reward: {: .2f} | status: {}".format(
    ...
    def render_q(self, model):
        ...
        if self.__render == Render.TRAINING:
            ...
            self.__ax2.text(*self.__exit_cell, "Exit",
                            ha="center", va="center", color="white")
            ...
            for cell in self.empty:
                ...
                for action in a:
                    ...
                    self.__ax2.arrow(
                        *cell, dx, dy, color=(1 - color, color, 0), head_width=0.2, head_length=0.1)
